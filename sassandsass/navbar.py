...
def generate_navbar():
    ...
    navbar = [dict(page=get_page_descriptor(row[0]), children=get_page_descriptor(row[1])) 
             for row in pages.fetchall()]
    return navbar

def get_page_descriptor(ids):
    pages = ids.split(',')
    descriptors = g.db.execute("SELECT title, link_alias FROM pages WHERE id IN (?);", ids)
